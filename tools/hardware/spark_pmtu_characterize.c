...
static int SparkPmtuRunServer(const SparkPmtuOptions *options)
{
    ...
    while (SparkPmtuStopRequested == 0)
    {
        ...
        if (fingerprint != be64toh(header->fingerprint))
        {
            continue;
        }
...
static void SparkPmtuDrainSocket(int socket_descriptor)
{
    ...
    for (;;)
    {
        ...
        received = recv(socket_descriptor, discard, sizeof(discard), MSG_DONTWAIT);
        if (received >= 0)
        {
            continue;
        }
    ...
    uint64_t *latencies_out)
{
    ...
    for (iteration = 0u; iteration < iterations; ++iteration)
    {
        ...
        header->fingerprint = htobe64(SparkPmtuPayloadFingerprint(
        ...
        sent = sendto(socket_descriptor, packet, packet_bytes, 0,
            (const struct sockaddr *)peer_address, peer_length);
...
static int SparkPmtuRunClient(const SparkPmtuOptions *options)
{
    ...
    if (setsockopt(socket_descriptor, IPPROTO_IP, IP_MTU_DISCOVER,
        ...
        connect(socket_descriptor, (struct sockaddr *)&peer_address, peer_length) != 0)
    {
    ...
    while (low <= high)
    {
        ...
        if (SparkPmtuTryPayload(socket_descriptor, &peer_address, peer_length,
                middle, options->iterations, latencies))
        {
            best = middle;
    ...
    memset(latencies, 0, (size_t)options->iterations * sizeof(uint64_t));
    if (!SparkPmtuTryPayload(socket_descriptor, &peer_address, peer_length,
            best, options->iterations, latencies))
    {
