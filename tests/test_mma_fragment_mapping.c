...
// row_index[clamp(p)] with one bulk copy per 16-byte swizzle chunk instead of
// one TMA box. The numerics-identical claim is a BYTE claim: for every live
// row the staged tile must equal what gather-plus-box stages, because the
// fragment loads and the accumulate order downstream are shared code. Both
// stagings are address arithmetic over a tagged source, so a host can check
// them directly - the poisoned indices past row_limit included, since the
// clamp is the difference between dead traffic and a wild copy.
//
