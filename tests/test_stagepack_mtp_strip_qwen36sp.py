...
def synthetic_corrupted_pack(directory: Path) -> Path:
    """The measured fleet corruption: a stripped rank-3 pack whose header
    reads mtp=1 tpdeg=4 tprank=0 (the old strip zeroed u32[25])."""
    header_values = {
        "magic": 0x50533651, "format_version": 3,
        "header_bytes": HEADER_BYTES, "entry_bytes": ENTRY_BYTES,
        "tensor_count": 1, "hidden_dimension": 5120,
        "layer_count": 4, "first_layer_index": 0,
        "total_layer_count": LAYER_COUNT, "attention_period": 4,
        "full_attention_phase": 3, "gdn_key_head_count": 16,
        "gdn_value_head_count": 48, "gdn_head_key_dimension": 128,
        ...
        "ffn_intermediate_dimension": 17408,
        "output_vocab_count": 248320, "mxfp4_group_size": 32,
        "mtp_layer_count": 1, "tp_degree": 4, "tp_rank": 0,
    }
    payload_offset = 256
    entries = [
        (5, 7, 0, 1, 64, payload_offset, 128, 0, 0),
    ...
    body = HEADER_STRUCT.pack(
        *(header_values[name] for name in HEADER_FIELDS),
        DIRECTORY_OFFSET, payload_offset + 128)
    body += b"".join(struct.pack("<6I", kind, layer, fmt, rows, cols, 0)
                     + struct.pack("<4Q", poff, pbytes, soff, sbytes)
                     for kind, layer, fmt, rows, cols, poff, pbytes, soff, sbytes
                     in entries)
    body += b"\0" * (payload_offset - len(body))
    body += b"\xa5" * 128
    pack = directory / "qwen27b.tp4.rank3.qwen36sp"
    pack.write_bytes(body)
    return pack


def test_family_map_carries_tp_field_indices():
    strip = load_strip_tool()
    family = strip.FAMILIES["qwen36sp"]
    assert family["tp_degree_index"] == TP_DEGREE_INDEX == 24
    assert family["tp_rank_index"] == TP_RANK_INDEX == 25


def test_repair_header_restores_rank_field():
    strip = load_strip_tool()
    with tempfile.TemporaryDirectory() as tmp:
        pack = synthetic_corrupted_pack(Path(tmp))
        before = HEADER_STRUCT.unpack(pack.read_bytes()[:HEADER_BYTES])
...
def test_repair_header_refuses_clean_and_unknown_states():
    strip = load_strip_tool()
    with tempfile.TemporaryDirectory() as tmp:
        pack = synthetic_pack(Path(tmp))
...
def main() -> int:
    ...
    test_family_map_carries_tp_field_indices()
    test_strip_removes_mtp_tail_and_preserves_tp_fields()
    test_repair_header_restores_rank_field()
    test_repair_header_refuses_clean_and_unknown_states()
    print("test_stagepack_mtp_strip_qwen36sp: 5 checks PASS")
    return 0
